...
def run_case(
    ...
    llm.exit()
    metrics = {
        "mode": "int8-kv" if kv_cache_quant else "baseline",
        "time": elapsed,
...
def print_summary(results: list[dict], prompt_len: int, max_tokens: int):
    ...
    base, quant = results
    print()
    print(f"KV block gain: {quant['num_kvcache_blocks'] / max(base['num_kvcache_blocks'], 1):.2f}x")
    print(f"Estimated live-sequence gain: {quant['est_max_live_seqs'] / max(base['est_max_live_seqs'], 1):.2f}x")
    print(f"Init allocated delta: {quant['init_allocated_gb'] - base['init_allocated_gb']:+.3f} GB")
    print(f"Generate peak delta: {quant['generate_peak_gb'] - base['generate_peak_gb']:+.3f} GB")
    print(f"Runtime temporary delta: {quant['runtime_peak_delta_gb'] - base['runtime_peak_delta_gb']:+.3f} GB")
...
def main():
    ...
    results = [
        run_case(args.model_path, prompts, args.max_tokens, False, max_model_len, max_num_batched_tokens),
        run_case(args.model_path, prompts, args.max_tokens, True, max_model_len, max_num_batched_tokens),
    ]
